...
abstract class GenericClient
{
    ...
    /**
     * Set your system information, system name and system version
     * @param $systemName
     * @param $systemVersion
     * @return \Sellermania\GenericClient
     * @throws \Sellermania\Exception
     */
    public function setSystemInfo($systemName,$systemVersion)
    {
        if (!is_string($systemName))
        {
            throw new Exception(sprintf("systemName should be a string, variable of %s type given", gettype($systemName)));
        }
        if (!is_string($systemVersion))
        {
            throw new Exception(sprintf("systemVersion should be a string, variable of %s type given", gettype($systemVersion)));
        }
    ...
    protected function operation($operation, array $params = array ())
    {
        ...
        $this->client = new MTOMSoapClient($this->endpoint, array(
            ...
            'stream_context' => stream_context_create(array(
                'http' => array(
                    'header' =>sprintf("customer-system-name:%s",$this->systemName).PHP_EOL.
                        sprintf("customer-system-version:%s",$this->systemVersion)
                ),
            )),
        ));
